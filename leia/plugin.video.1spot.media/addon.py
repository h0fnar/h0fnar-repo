...
# xbmc.log("++++++++++ kwargs =" + str(kwargs), level=xbmc.LOGNOTICE)
...
def list_series():
    ...
    while i <= max_guesses: 
        ...
        title = parsed_json[i]['title']
        #title = "[COLOR darkorchid][B]" + title + "[/B][/COLOR]"   
...
def list_season(sid, poster, banner, description):
    ...
    results = parsed_json['seasons'] # need to be fixed
    # fix:
    nkeys =[]
    ...
    while i <= max_guesses:
        s_id = nkeys[i] # fix
        #staffel = parsed_json['seasons'][i]['title']
        staffel = parsed_json['seasons'][s_id]['title'] # fix


        #url = get_url(action='episode', sid=sid, poster=poster, banner=banner, season=i)
        url = get_url(action='episode', sid=sid, poster=poster, banner=banner, season=s_id) # fix
        is_folder = True
