...
import requests
import json
#import commands # pyton3 update
import subprocess # pyton3 update
...
host = subprocess.getoutput('ifconfig '+interface+' | grep "inet\ addr" | cut -d: -f2 | cut -d" " -f1') # pyton3 update
...
#entries = filter(None, entries) # pyton3 update
