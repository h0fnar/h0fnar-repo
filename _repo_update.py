...
def get_repo_name():
    """
    Get the ID of the repository from the addons.xml
    """
    top = os.getcwd()
    addons_xml_path = top + "/repo/zips/addons.xml"
    addons_xml = ElementTree.parse(addons_xml_path)
    repository = addons_xml.find(".//addon").attrib["id"]
    return repository


def delete_zip_folders():
    """
    Delete all zips folders
    """
    top = os.getcwd()
    byte_count = 0
    folders_count = 0
    try:
        for root, dirs, files in os.walk(top, topdown=False):
            for dir in dirs:
                if dir == 'zips':
                    ...
                    dir_temp = dir_path.replace(top, '')
                    dir_size = get_dir_size(dir_path)
                    dir_size = convert_bytes(dir_size)
        ...
        byte_count = convert_bytes(byte_count)
        if folders_count > 0:
            print(
                "{} {} - {}".format(
                    color_text("Successfully removed", 'yellow'),
        ...
        print(
            "{}\n{}".format(
                color_text("An error occurred removing zip folders", 'red'),
                color_text(e, 'red')
            )
        )
...
def delete_repo_zip():
    """
    Delete the repository.zip in the root folder
    """
    top = os.getcwd()
    try:
        for root, dirs, files in os.walk(top):
            ...
            for file in files:
                ...
                if file.endswith('.zip'):
                    ...
                    repo = os.path.join(root, file)
                    size = convert_bytes(os.path.getsize(repo))
        ...
        print(
            "{}\n{}".format(
                color_text("An error occurred removing repository", 'red'),
                color_text(e, 'red')
            )
        )


def copy_repo_zip(repository):
    """
    Copy the repository.zip to the root folder
    """
    root_dir = os.getcwd()
    top = root_dir + "/repo/zips/" + repository
    try:
        for root, dirs, files in os.walk(top):
            # select file name
            for file in files:
        ...
        print(
            "{} {}\n{}".format(
                color_text("An error occurred copying repository", 'red'),
                color_text(repository, 'yellow'),
                color_text(e, 'red')
            )
        )
